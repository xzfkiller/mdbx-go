...
import (
	...
	"github.com/xzfkiller/mdbx-go/mdbx"
)
...
// methods.  However, even though TxnPool can only reuse mdbx.Readonly
// transactions it this way it should be used to create and terminate all Txns
// if it is used at all.  Update transactions can cause MDBX to use excessive
// numbers of pages when there are long-lived mdbx.Readonly transactions in a
...
// BeginTxn is analogous to the BeginTxn method on mdbx.Env but may only be
// used to create mdbx.Readonly transactions.  Any call to BeginTxn that does
// not include mdbx.Readonly will return an error
...
func (p *TxnPool) beginReadonly() (*mdbx.Txn, error) {
	txn, ok := p.pool.Get().(*mdbx.Txn)
	if !ok {
...
func (p *TxnPool) handleReadonly(txn *mdbx.Txn, condition UpdateHandling) (renewed bool, err error) {
...
// only be passed mdbx.Txn objects which it returned from a call to BeginTxn.
